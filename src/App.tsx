...
export default function App() {
  ...
  return (
    <div className={`min-h-[100dvh] overflow-hidden bg-[#080910] text-white font-ios selection:bg-white/20 flex flex-col items-center transition-all duration-700 ease-ios ${isTransitioning ? "opacity-0 scale-[0.98] blur-xl" : "opacity-100 scale-100 blur-0"}`}>
      <style>{`
        .font-ios { font-family: -apple-system, BlinkMacSystemFont, "SF Pro Display", "SF Pro Text", Inter, Segoe UI, sans-serif; }
        .font-luxury { font-family: Georgia, "Times New Roman", serif; }
        .ease-ios { transition-timing-function: cubic-bezier(0.22, 1, 0.36, 1); }
      ...
      {selectedTransaction && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60 backdrop-blur-2xl p-4 sm:p-6 transition-all" onClick={() => setSelectedTransaction(null)}>
          <div className="m-auto w-full max-w-sm rounded-[2.2rem] p-5 sm:p-6 glass-panel anim-pop-in max-h-[82dvh] overflow-y-auto scrollbar-hide shadow-[0_40px_100px_rgba(0,0,0,0.6)]" onClick={(e) => e.stopPropagation()}>
